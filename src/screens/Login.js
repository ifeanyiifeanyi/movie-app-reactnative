...
export default function Login({ route }) {
  ...
  const login = async (username, password) => {
    if (!username.trim() || !password.trim()) {
      ...
      axios
        .post(`${EXPO_PUBLIC_BASE_URL}/api/login`, {
          username: username,
          password: password,
          ...
          if (response.data.status) {
            ...
            if (user_plan_expire) {
              AsyncStorage.setItem(
                "user_plan_expire",
                JSON.stringify(user_plan_expire)
              )
                .then(() => {
            ...
            try {
              await AsyncStorage.setItem(
                "uid",
                JSON.stringify(response.data.username.id)
              );
              ...
              await AsyncStorage.setItem(
                "status",
                JSON.stringify(response.data.username.status)
              );
              // remember to use this to hide videos if null
              await AsyncStorage.setItem(
                "subscription_id",
                JSON.stringify(response.data.username.subscription_id)
              );
            } catch (error) {
              console.log(error);
            }
            ...
            if (response.data.username.status == 1) {
              ...
              setTimeout(() => {
                navigation.navigate("SelectUser", {
                  user_id: response.data.username.id,
                });
                setSuccess("");
              }, 4000);
            } else {
  ...
  return (
    <View style={styles.container}>
      ...
      {success && (
        <Text
          style={{ color: "limegreen", marginBottom: 20, fontWeight: "900" }}
        >
          {" "}
          {success}
        </Text>
      ...
      >
        ...
        <TouchableOpacity onPress={() => setShowPassword(!showPassword)}>
          <Image
            source={
              !showPassword
                ? require("../img/logo/eye.png")
                : require("../img/logo/close-eyes.png")
            }
